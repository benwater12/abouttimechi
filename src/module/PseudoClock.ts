...
import { Quentry } from "./FastPirorityQueue";
...
export class PseudoClock {
  ...
  private static _realTimeInterval: number;
  ...
  static _initialize(currentTime: number = 0, realTimeMult: number = 0,realTimeInterval: number, running: Boolean = false) {
    ...
    if (!CONFIG.time.roundTime) CONFIG.time.roundTime = 6;
  }
  ...
  static demote() {
    ...
    if (PseudoClock._realTimeTimerID) clearInterval(PseudoClock._realTimeTimerID)
  }

...
import { DTMod } from "./calendar/DTMod";
  ...
  var defaultcomparator = function(a:Quentry, b: Quentry) {
...
export class Quentry {
  ...
  constructor(time: number,  recurring: Boolean, increment: DTMod, handler: (...args) => {}, uid, ...args) {
    this._time = time;
  ...
  static createFromJSON(data): Quentry {
    let handler;
    try {
      if (data.handler.type === "function") handler = eval(data.handler.val)
...
export class FastPriorityQueue {
  ...
  constructor(comparator: (a:any, b:any) => Boolean = defaultcomparator)  {
  ...
  static createFromJson(data): FastPriorityQueue {
  ...
  _batchRemove = function(callback, limit) {
  ...
  removeMany = function(callback, limit) {

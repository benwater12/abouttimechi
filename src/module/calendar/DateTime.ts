...
export class DateTime extends DTMod {
  ...
  rawAdd(increment: DTMod) : DateTime {
   ...
   */
  add(increment: DTMod): DateTime {
    this.rawAdd(increment);
    this.normalize();
    return this;
  }

  setAbsolute({years=null, months=null, days=null, hours=null, minutes=null, seconds=null}={}): DateTime {
   ...
   * @param startYear 
   */
  static createFromSeconds(seconds: number, startYear = DTCalc.clockStartYear): DateTime {
    if (DTCalc.debug) DTCalc.log("craeate from seconds", DateTime.create({years: startYear, seconds: seconds}), DateTime.create({years: startYear, seconds: seconds}).normalize())
 ...
 */
  toDays() :{days: number, time: DTMod} {
    let days = this.days;
    let monthsPerYear = DTCalc.dpm.length;
 
    // roll up the time part to days.
    let seconds: number = DTCalc.timeToSeconds({hours: this.hours, minutes: this.minutes, seconds: this.seconds});
    ...
    if (DTCalc.debug) DTCalc.log("DateTime toDays", days,  DateTime.create({days, hours: time.hours, minutes: time.minutes, seconds: time.seconds}))
   ...
   * @param d2 
   */
  static daysBetween(d1: DateTime, d2: DateTime): number {
   ...
   */
  setCalDow(dow: number) {
    if (DTCalc.debug) DTCalc.log("DateTime: setting first day ", dow, this.dow(), DTCalc.firstDay)
  ...
  toSeconds(): number 
  {
    if (DTCalc.debug) DTCalc.log("calcseconds", this.years, this.months, this.days, this.hours, this.minutes, this.seconds);
   ...
   */
  shortDate(): {"date": string, "time": string} {
  ...
  longDateSelect({day = true, month = true, year = true, hours = true, minutes = true, seconds = true, monthDay = true}): {"date": string, "time": string} {
    let pad = DTCalc.padNumber;
    let yearD = this.years - (this.years < 1 && !DTCalc.hasYearZero ? 1 : 0);

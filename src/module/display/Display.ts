...
import { CalendarEditor } from "../calendarEdtior/CalendarEditor";
import { DTCalc, calendars } from "../calendar/DTCalc";


let displayMain:SimpleCalendarDisplay = null;
export class SimpleCalendarDisplay extends FormApplication {
  constructor(object = {}, options:FormApplicationOptions = null) {
  ...
  activateListeners(html) {
    ...
    .click(event => {       
      if (["0.5.1", "0.5.0"].includes(game.data.version)) {
        ...
        //@ts-ignore
        var calendar_weather = game.modules.get('calendar-weather')?.active;
      }
      if (!calendar_weather) new CalendarEditor(
    ...
    if (PseudoClock.isMaster) {
      ...
      .click(event => {
        if (PseudoClock.isRunning()) PseudoClock.stopRealTime();
        else PseudoClock.startRealTime();
        this.render(false);
      });
    }
  }
